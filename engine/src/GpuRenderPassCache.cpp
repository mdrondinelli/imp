...
namespace mobula {
  ...
  auto GpuRenderPassCache::create(GpuRenderPassCreateInfo const &createInfo)
      -> GpuRenderPass const & {
    auto lock = std::scoped_lock{mutex_};
    if (auto it = renderPasses_.find(createInfo); it != renderPasses_.end()) {
      return it->second;
    }
    auto ownedAttachmentDescriptions = std::vector<GpuAttachmentDescription>(
    ...
    auto ownedSubpassDependencies = std::vector<GpuSubpassDependency>(
