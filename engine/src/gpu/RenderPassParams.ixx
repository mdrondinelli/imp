...
export module mobula.gpu:RenderPassParams;
import <vector>;
// clang-format on
...
namespace mobula {
  namespace gpu {
    ...
    hash_value(RenderPassParams::SubpassDescription const &desc) noexcept {
      ...
      hash_range(
          seed, begin(desc.inputAttachments), end(desc.inputAttachments));
      hash_range(
          seed, begin(desc.colorAttachments), end(desc.colorAttachments));
